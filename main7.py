...
def move_cursor(index_tip):
    ...
    if index_tip is not None:
        x_coord = int(index_tip.x * screen_w * sensitivity)
        y_coord = int(index_tip.y / 2 * screen_h * sensitivity)

        # Smooth the cursor movement
        x_coord = alpha * x_coord + (1 - alpha) * prev_x
        y_coord = alpha * y_coord + (1 - alpha) * prev_y
